...
class OAuthClient extends EventEmitter {
  ...
  async _runRegister(discovered) {
    ...
    const existingClientId = await this.identityProvider.getClientId(this.siteUrl);
    if (existingClientId) {
      this._progress('reusing_persisted_client_id', { clientId: existingClientId });
      // We still need the redirect_uri-bound loopback port to match what was
      // registered. Forcing a fresh DCR (which v1.0 always does because
      // FreshEachTime returns null) sidesteps that — see Appendix H.3.2.
      return { clientId: existingClientId, raw: { reused: true } };
    }
