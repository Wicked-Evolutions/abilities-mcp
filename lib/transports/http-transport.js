      ...
      if (result.body && result.body.trim()) {
        ...
        if (Array.isArray(batchResponse)) {
          for (let resp of batchResponse) {
            if (this.clientProtocolVersion && resp.result && resp.result.protocolVersion) {
              resp.result.protocolVersion = this.clientProtocolVersion;
            }
  ...
  async _processMessage(line) {
    ...
    try {
      ...
      if (result.body && result.body.trim()) {
        ...
        // Protocol version rewriting — match client's version
        if (this.clientProtocolVersion && parsed.result && parsed.result.protocolVersion) {
          parsed.result.protocolVersion = this.clientProtocolVersion;
        }
