...
class SshTransport {
  ...
  send(line) {
    ...
    if (this.child && this.childReady && !this.child.killed) {
      ...
      try {
        const msg = JSON.parse(line);
        if (msg.method === 'initialize' || msg.method === 'initialized' || msg.method === 'notifications/initialized') {
          this.child.stdin.write(line + '\n');
  ...
  _remoteCleanup() {
    try {
      const killCmd = `pkill -f 'mcp-adapter serve --server=${this.mcpServer}'`;
      this.log(`[${this.host}] Remote cleanup: ${killCmd}`);
      execSync(`ssh -o BatchMode=yes -o ConnectTimeout=5 ${this.host} "${killCmd}"`, {
        timeout: 10000,
        stdio: 'ignore',
      });
    } catch (e) {
