...
class ConnectionPool {
   ...
   * failed site's stale transport, then connect the first healthy *other*
   * site and promote it to runtime default. Returns the new transport, or
   * `null` when no other site can serve (the genuine "all sites failed"
   * condition — the caller then enters degraded mode per the #76 gate).
   *
   * @param {string} failedSiteId  The site whose initialize forward failed.
   * @param {function} onMessage   Per-site message router callback.
   * @returns {Promise<Transport|null>}
   */
  async connectFallback(failedSiteId, onMessage) {
    const stale = this.transports.get(failedSiteId);
    if (stale) {
      try { await stale.shutdown(); } catch { /* best effort — stale anyway */ }
      this.transports.delete(failedSiteId);
    }

    const tryOrder = Object.keys(this.config.sites).filter((k) => k !== failedSiteId);
    for (const key of tryOrder) {
      try {
        const transport = await this._createTransport(key, null);
        transport.onMessage = onMessage;
        ...
        this.config.defaultSite = key;
        this.log(
          `Default "${failedSiteId}" failed at request time; runtime default ` +
          `fell back to "${key}". "${failedSiteId}" is marked degraded ` +
          `in-memory; reauth it to restore.`
        );
        return transport;
      } catch (err) {
