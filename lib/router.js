...
class McpRouter {
   ...
   * default site (request-time refresh-token expiry). Try a healthy fallback
   * site and re-forward the cached initialize through it so the client gets a
   * real InitializeResult and the tool catalog populates normally — the bridge
   * stays healthy and only the failed site is marked degraded (visible via
   * wp_bridge_health). Enter degraded mode ONLY when no site can serve, which
   * preserves the #76 gate (no init-time bridge death) for the genuine
   * all-sites-down case.
   *
  ...
  async _handleInitializeForwardFailure(failedSite, reason) {
    let fallback = null;
    try {
      fallback = await this.pool.connectFallback(failedSite, (parsedMsg, rawLine) => {
    ...
    if (fallback) {
      ...
      this.log(
        ...
        `"${this.config.defaultSite}". Bridge NOT degraded (Issue #87 S1).`
      );
      this._forwardToDefault(JSON.stringify(this.cachedInitRequest));
      return;
    }

    // No healthy site — genuine all-sites-down. Preserve the #76 gate:
    ...
    // per-site status. degradedSites mirrors the boot path (abilities-mcp.js):
    // every configured site, failed default carrying the live error reason.
    this.log('No healthy fallback site — entering degraded mode (all sites failed, #76 gate).');
    const degradedSites = Object.entries((this.config && this.config.sites) || {}).map(
      ([siteId, site]) => ({
        siteId,
        reason: siteId === failedSite
          ? reason
