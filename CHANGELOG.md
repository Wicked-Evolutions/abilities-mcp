...
- **Multisite auto-populate via `add-site` may fail on multisite-network OAuth setups.** v1.5.4 introduced the `add-site` flow that probes the network root for its subsite list and writes the `multisite` block automatically. The probe uses `BearerJsonRpcClient` (a one-shot JSON-RPC client distinct from the long-running MCP runtime session) which captures `Mcp-Session-Id` from the `initialize` response but not the `Mcp-Session-Token` HMAC the adapter requires on every subsequent request to prevent session fixation. The adapter's session-validation contract at `HttpSessionValidator::validate_session()` is intentional defense, so the second call (`multisite/list-sites`) is correctly rejected. **The fix belongs in this bridge, not in the adapter** — `BearerJsonRpcClient` needs to capture and echo the `Mcp-Session-Token` header from the `initialize` response on every subsequent request. Tracked in [#54](https://github.com/Wicked-Evolutions/abilities-mcp/issues/54). Operator workaround in the meantime: hand-edit `~/.abilities-mcp/wp-sites.json` to add the `multisite` block manually after `add-site` writes the OAuth entry — see `wp-sites.example.json` for the expected shape. Once the OAuth entry exists with a populated `multisite` block, **subdomain-style multisite OAuth routing (the v1.5.5 #48 fix) works end-to-end** — the gap is in the auto-populate probe, not in the routing fix it ships alongside. Adapter-side context for why this presents as a 4xx rather than a typed retry is captured in [adapter #87](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/87) (closed-as-deferred per the adapter v1.4.5 §9 Q6 escape hatch).
