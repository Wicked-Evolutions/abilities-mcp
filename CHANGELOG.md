...
- **Cross-process keychain ACL prompts on macOS (Issue [#58](https://github.com/Wicked-Evolutions/abilities-mcp/issues/58)).** When the bridge writes a keychain entry, macOS's default ACL trusts only the creating process. The bridge has multiple consumer processes — Claude Desktop's `.mcpb`-bundled runtime, Claude Code's source bridge, terminal CLI invocations (npm-linked, Homebrew, source-direct), and the `keytar` native binding fallback — each of which is a different process identity to macOS. Cross-process keychain reads trigger an ACL approval dialog on first access. Operator workaround: click **Always Allow** on the first prompt for each consumer process; macOS persists the approval for that specific binary path going forward. Tracked in #58 for v1.5.6 with a deliberate trust strategy (explicit `-T` consumer-path list at write time, or canonical helper identity). Shipping the proper architectural fix at v1.5.5 release-edge was assessed as high risk by GPT 5.5 final-review (touches multiple binary identities, packaging paths, installer state) — deferred to give it room to test across the full consumer-process surface. The cross-process gap is UX, not security: existing keychain ACLs continue to protect secrets correctly; operators just see one extra approval click per consumer.
